...
class User(Resource):
    ...
    def get(self):
        ...
        def create_request_parser():
            self.parser = reqparse.RequestParser()
            self.parser.add_argument('page_no', required=False, location='args', default=1)
            return self.parser
        create_request_parser()
        args = self.parser.parse_args()
        extension_access_token = get_extension_access_token()
        ...
        per_page = 10
        routes = 'groups' + '/' + Configuration.AUTH0_INFINERA_GROUP_ID + '/' + 'members' + '?per_page=' + str(
            per_page) + '&page=' + str(args['page_no'])
        ext_url = Configuration.AUTH0_EXTERNAL_API + routes
        response = requests.get(ext_url, headers=headers)
